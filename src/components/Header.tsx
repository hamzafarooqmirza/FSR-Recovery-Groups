          ...
          <nav className="hidden items-center gap-7 text-sm font-semibold lg:flex" aria-label="Main navigation">
            {navLinks.map((link) => link.href === "/services" ? (
              ...
              <div key={link.label} className="group relative">
                <button
                  type="button"
                  className={`nav-arrow flex items-center gap-1.5 py-7 transition hover:text-red focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-red ${activePath.startsWith("/areas") ? "text-red" : "text-white/85"}`}
                ...
                >
                  Areas <i className={`fa-solid fa-chevron-down text-[10px] transition ${isAreasOpen ? "rotate-180" : ""}`} />
                </button>
                <div
                  id="desktop-areas-menu"
                  role="menu"
                  className={`absolute left-1/2 top-full z-50 w-[34rem] max-w-[90vw] -translate-x-1/2 rounded-2xl border border-white/10 bg-navy p-4 shadow-2xl transition duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100 ${isAreasOpen ? "visible translate-y-0 opacity-100" : "invisible -translate-y-2 opacity-0"}`}
                >
                  <div className="grid max-h-[70vh] gap-x-6 gap-y-5 overflow-y-auto sm:grid-cols-2">
                    <div>
                      ...
                      <div className="grid gap-0.5">
                        {cityAreas.map((area) => (
                          <Link key={area.key} href={area.href} role="menuitem" className="block rounded-xl px-3 py-2 text-sm text-white/80 transition hover:bg-white/10 hover:text-red focus-visible:outline-2 focus-visible:outline-red" onClick={() => setIsAreasOpen(false)}>{area.label}</Link>
                        ))}
                      </div>
                    </div>
                    <div>
                  ...
                  <Link href="/areas" role="menuitem" className="mt-4 block rounded-xl border-t border-white/10 px-3 pt-4 text-center font-semibold text-red transition hover:text-red-dark focus-visible:outline-2 focus-visible:outline-red" onClick={() => setIsAreasOpen(false)}>View All Areas</Link>
                </div>
              </div>
            ) : (
              <a
            ...
            <div className="container-site max-h-[calc(100dvh-7rem)] overflow-y-auto py-4 font-semibold">
              {navLinks.map((link) => link.href === "/services" ? (
                ...
                <div key={link.label}>
                  <button
                    type="button"
                    className={`flex w-full items-center justify-between rounded-lg py-3 text-left ${activePath.startsWith("/areas") ? "text-red" : "text-white/85"}`}
                    aria-expanded={isMobileAreasOpen}
                    aria-controls="mobile-areas-menu"
                    onClick={() => setIsMobileAreasOpen((open) => !open)}
                  >
                    <span><i className="fa-solid fa-chevron-right mr-2 text-xs text-red" />Areas</span>
                    <i className={`fa-solid fa-chevron-down mr-2 text-xs transition ${isMobileAreasOpen ? "rotate-180" : ""}`} />
                  </button>
                  {isMobileAreasOpen && (
                    <div id="mobile-areas-menu" className="ml-3 grid gap-1 border-l border-red/30 pb-2 pl-4">
                      <Link href="/areas" className="rounded-lg px-3 py-2.5 text-white hover:bg-white/5 hover:text-red" onClick={() => setIsMenuOpen(false)}>View All Areas</Link>
                      <p className="mt-2 px-3 text-xs font-bold uppercase tracking-wider text-white/40">Cities &amp; Towns</p>
                      {cityAreas.map((area) => (
                        <Link key={area.key} href={area.href} className="rounded-lg px-3 py-2.5 text-sm text-white/75 hover:bg-white/5 hover:text-red" onClick={() => setIsMenuOpen(false)}>{area.label}</Link>
                      ))}
                      <p className="mt-2 px-3 text-xs font-bold uppercase tracking-wider text-white/40">Main Roads</p>
                      {roadAreas.map((area) => (
                        <Link key={area.key} href={area.href} className="rounded-lg px-3 py-2.5 text-sm text-white/75 hover:bg-white/5 hover:text-red" onClick={() => setIsMenuOpen(false)}>{area.label}</Link>
                      ))}
                    </div>
